	...
	foreach($dbCategories as $dbCategory){
		...
		if($displayCategoryBool){
			...
			if($subMenuStr!="" && $subMenuStr!='<ul class="dropdown-menu"></ul>'){
				$menuHTMLStr .=  '<a href="products.php?category='.$dbCategory['uuid'].'">'.ucfirst($dbCategory['name']).'<span class="caret"></span></a>';
				$menuHTMLStr .=  $subMenuStr;
			}else{
				$menuHTMLStr .=  '<a href="products.php?category='.$dbCategory['uuid'].'">'.ucfirst($dbCategory['name']).'</a>';
			}
			$menuHTMLStr .=  '</li>';
	...
	<div CLASS="row" STYLE="padding: 7px 20px; width:100%;">
		<div CLASS="col-md-12 text-right" STYLE="padding-right:0px;">
			<?php if(isset($isUserSignedInBool) && $isUserSignedInBool==true && isset($userLoggedIn)){	?>
				<a HREF="myaccount.php" STYLE="color:#000; font-size: 13px;" >Hi, <?php echo $userLoggedIn['First name']." ".$userLoggedIn["Surname"];	?></a> | <a HREF="logout.php" STYLE="color:#000; font-size: 13px;" > <i CLASS="fa fa-sign-in" ></i> Logout</a>
			<?php } else	{	?>
				<a HREF="login.php" STYLE="color:#000; font-size: 13px;" > <i CLASS="fa fa-sign-in" ></i> Sign In</a> | <a HREF="register.php" STYLE="color:#000; font-size: 13px;" > <i CLASS="fa fa-edit"></i> Register</a>
			<?php } ?>
			<!-- |
			<a HREF="#" STYLE="color:#000; font-size: 13px;" > <i CLASS="fa fa-sign-out"></i> Sign Out</a>-->
			<a HREF="cart.php" CLASS="btn btn-danger btn-sm displayCartsClass" STYLE="margin-left:7px; display:none;"> <i CLASS="fa fa-cart-plus"></i> &nbsp; Cart <span class="cart_items cartItemsClass">0</span></a>
		</div>
	</div>
</div>
<header>
	<div class="navbar navbar-default" role="navigation" style="margin-bottom:0px;">
        ...
        <div class="navbar-collapse collapse main-nav">
        	<div STYLE="float:right;" CLASS="hidden-sm hidden-xs">
		  		<form id="mainSearch" class="searchnew" action="products.php" method="post">
			...
			<div class="visible-sm visible-xs" style="margin-top:20px;">
				<form id="mainSearch" role="search" action="products.php" method="post">
  			...
  			<!--<div CLASS="visible-sm visible-xs" STYLE="margin-top:20px;">
 				<form id="search" role="search" action="products.php" method="post">
